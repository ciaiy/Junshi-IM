...
namespace im
{
	namespace common
	{
		...
		#define catMsg(msg) string(msg).append(" {fileName:").append(__FILE__).append(" functionName:").append(__func__).append(" lineNumber:").append(int2string(__LINE__)).append("}").c_str()
				//单例类，只有一个对象
				static myLog *logger = myLog::getInstance();
		#define logSetpriority(priority) logger->setPriority(priority)
		#define logError(msg) logger->error(catMsg(msg))
		#define logWarn(msg) logger->warn(catMsg(msg))
		#define logInfo(msg) logger->info(catMsg(msg))
		#define logDebug(msg) logger->debug(catMsg(msg))
		#define logFatal(msg) logger->fatal(catMsg(msg))
